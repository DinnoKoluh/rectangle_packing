...
def get_available_area(w, L, D):
    ...
    model.add_string(
        ...
        constraint forall(i in 1..n) (X[i] >= 0 /\ X[i] + D[i,1] <= w);
        constraint forall(i in 1..n) (Y[i] >= 0 /\ Y[i] + D[i,2] <= L);

        constraint forall(i in 1..n, j in i+1..n) (X[i] + D[i,1] <= X[j] \/ X[j] + D[j,1] <= X[i] \/ Y[i] + D[i,2] <= Y[j] \/ Y[j] + D[j,2] <= Y[i]);
